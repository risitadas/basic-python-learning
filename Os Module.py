...
---------Built-in Functions and their Workings---------
...
It is used in case we want to change our directory. The new path is sent inside the parenthesis.
...
To rename an already existing directory, we use this.
...
It will return us the environment variable. The environment variable must be set,
 or the function will return null.

os.path.join( ):

It joins one or more path components. We can join the paths by simply
using a + sign, but the benefit of using this function is that we do not have to worry
about extra slashes between the components. So less accuracy still provides us with the same result.
...
# os.rename("file5.txt", "filee5.txt")
# print(os.environ.get('Path'))
# print(os.path.join("C:/", "/file5.txt"))
